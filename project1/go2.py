...
class Group:
    ...
    def search_nearby_groups(self, state, game, piece, player=-1, board_init=False):
        ...
        # Check if the space at the right of the piece exists
        if piece_column < game.boardSize:
            self.search_sides(player,real_group, state, game, piece, groupsDofChanged, groupsJoined, 'right', board_init)

        # Check if the space at the left of the piece exists
        if piece_column > 1:
            self.search_sides(player,real_group,state, game, piece, groupsDofChanged, groupsJoined, 'left', board_init)

        # Check if the space above the piece exists
        if piece_row > 1:
            self.search_sides(player,real_group,state, game, piece, groupsDofChanged, groupsJoined, 'above', board_init)
            
        # Check if the space bellow the piece exists
        if piece_row < game.boardSize:
            self.search_sides(player,real_group,state, game, piece, groupsDofChanged, groupsJoined, 'below', board_init)
        ...
        if game.get_board_space(state, side_config) != 0:
            ...
            and not game.get_board_space(state, side_config) in groupsJoined: 
                ...
                for group in game.groups:
                    if group.id == game.get_board_space(state, side_config):                  
                        # Join the groups
